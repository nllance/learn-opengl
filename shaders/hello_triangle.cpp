...
int main(void) {
    ...
    // Unbind VBO, VAO, EBO, in order
    /* A VAO stores the `glBindBuffer `calls when the target is GL_ELEMENT_ARRAY_BUFFER
       This also means it stores its unbind calls so make sure not to unbind
       the element array buffer before unbinding the VAO, otherwise it doesn't have an EBO configured
    */
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glBindVertexArray(VAO);
        // Primitive type; the number of elements; type of the indices; offset 
